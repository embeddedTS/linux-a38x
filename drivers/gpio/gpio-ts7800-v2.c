                   ...
                   unsigned int offset)
{
   ...
   if (priv->res->base == NULL) {
        printk("%s %d, priv->res->base is NULL!\n", __func__, __LINE__);
        return -1;
   }
   ...
   if (offset < 26) {   /* DIO or LCD header,  */
      ...
      reg = readl(priv->res->base + 0x08);
      reg |= bit;
      writel(reg, priv->res->base + 0x08);
   } else if (offset < 58) {  /* pc/104 Row A */
      reg = readl(priv->res->base + 0x20);
      reg &= ~bit;
      writel(reg, priv->res->base + 0x20);
   } else if (offset < 83) {  /* pc/104 Row B */
      reg = readl(priv->res->base + 0x24);
      reg &= ~bit;
      writel(reg, priv->res->base + 0x24);
   } else if (offset < 101 ) { /* pc/104 Row C */
      reg = readl(priv->res->base + 0x28);
      reg &= ~bit;
      writel(reg, priv->res->base + 0x28);
   } else if (offset < 116) { /* pc/104 Row D */
      reg = readl(priv->res->base + 0x2C);
      reg &= ~bit;
      writel(reg, priv->res->base + 0x2C);
               ...
               unsigned int offset, int value)
{
   ...
   reg = readl(priv->res->base + reg_num);
   ...
   writel(reg, priv->res->base + reg_num);
   priv->direction[offset / 32] &= ~(1 << offset % 32);
   spin_unlock_irqrestore(&priv->lock, flags);
...
static int ts7800v2_gpio_probe(struct platform_device *pdev)
{
   ...
   /* The memory information is provided by drivers/mfd/ts78xx_mfd.c */
   priv->res = dev_get_drvdata(pdev->dev.parent);
   if (!priv->res ){
    return -ENODEV;
  }
   ...
   reg = readl(priv->res->base + 8) | 0x3fffffff;
   writel(reg, priv->res->base + 8);
   ...
   priv->gpio_chip.parent = priv->res->dev;
