...
#define MAX_TIMEOUT_MILLISECS 500
...
inline static void add_1readb_delay(struct ts7800v1_sdmmc_host *ts_sdmmc_host)
{
	readb(ts_sdmmc_host->base_iomem);
}

inline static void add_2readb_delay(struct ts7800v1_sdmmc_host *ts_sdmmc_host)
{
	readb(ts_sdmmc_host->base_iomem);
	readb(ts_sdmmc_host->base_iomem);
}
...
inline static u16 ts7800v1_sdmmc_ucrc16(u16 crc_in, u8 incr)
{
	u16 xor = crc_in >> 15;
	u16 out = crc_in << 1;
...
inline static u8 ts7800v1_sdmmc_crc7(u8 crc, const u8 *data, size_t len,
...
inline static void lowspeed_mkcommand(u8 cmdindx, u32 arg, u8 *retcmd)
{
...
inline static u8 get_clksel(struct ts7800v1_sdmmc_host *ts_sdmmc_host)
{
	return readb(ts_sdmmc_host->base_iomem + 0x2) & GENMASK(2, 0);
}

inline static void set_clksel(struct ts7800v1_sdmmc_host *ts_sdmmc_host,
			      u8 slot)
{
	u8 a;
 ...
 */
inline static void set_fifo_bit(u8 *pfifo, uint32_t cycle, u8 value)
{
 ...
 * this function assumes pfifo content are cleared prior to calling it
 */
inline static void set_fifo_bit_reversed(u8 *pfifo, uint32_t cycle, u8 value)
{
	u32 byte_indx = cycle >> 3;
...
inline static void set_fifo_nibble_reversed(u8 *pfifo, uint32_t nibble_cycle,
					    u8 value)
{
...
inline static void sd_cmd_write(struct ts7800v1_sdmmc_host *ts_sdmmc_host,
...
inline static void sd_cmd_read(struct ts7800v1_sdmmc_host *ts_sdmmc_host,
...
inline static void send_serialize_cmd(struct ts7800v1_sdmmc_host *ts_sdmmc_host,
				      struct ts7800v1_sdmmc_slot *pslot,
...
inline static void wait_for_response(struct ts7800v1_sdmmc_host *ts_sdmmc_host,
				     struct ts7800v1_sdmmc_slot *pslot,
				     bool low_speed)
{
	pslot->cmd_timeout = 0x0;
	for (pslot->cmd_timeout = 0; pslot->cmd_timeout < MAX_TIMEOUT_MILLISECS;
	     ++pslot->cmd_timeout) {
		u8 x;
				     ...
				     u32 *cmd_resp)
{
	...
	if (cmd_flags & MMC_RSP_PRESENT) {
		...
		if (pslot->cmd_timeout >= MAX_TIMEOUT_MILLISECS) {
	...
	unsigned int data_blksz, u32 data_offset, int *data_error)
{
	...
	u8 stat, resp_len_bytes, sent_resp_crc7, calc_resp_crc7,
		*dat0_sent_crc16_buf = NULL, *blk_buf = NULL;
	...
	blk_buf = kzalloc(sizeof(u8) * data_blksz, GFP_KERNEL);
	if (IS_ERR_OR_NULL(blk_buf)) {
		dev_warn(mmc_dev(ts_sdmmc_host->mmc_host),
			 "Error, Block buffer kzalloc failed with %ld\n",
			 PTR_ERR(blk_buf));
		*data_error = ret = -ENOMEM;
		goto done;
	}
	...
	if (!low_speed && (pslot->sd_state & DATSSP_4BIT)) {
		...
		read_sd_cmd_sd_dat_highspeed(
			ts_sdmmc_host, pslot->response, blk_buf,
		...
		if (pslot->blk_buf_cycle_indx != 0) {
			...
			/* check data crc16 */
			calc_crc16 = ts7800v1_sdmmc_crc16(blk_buf, data_blksz);
		...
		if (!low_speed && (pslot->sd_state & DATSSP_4BIT)) {
			...
			while ((stat & 0xf) == 0xf) {
				if (pslot->cmd_timeout++ >=
				    MAX_TIMEOUT_MILLISECS) {
					/* reset time-out before going to done, sine cmd may have been successful and only data-transfer failed */
					pslot->cmd_timeout = 0x0;
			...
			for (i = 0; i < blksz_dwords; i++) {
				...
				x32 = readl(ts_sdmmc_host->base_iomem +
					    SDCORE2_SDDATA_REG);
				memcpy(&blk_buf[i << 2], &x32, sizeof(u32));
			}
			...
			while ((stat & 0xf) == 0xf) {
				if (pslot->cmd_timeout++ >=
					...
					/* reset time-out before going to done,
                            * sine cmd may have been successful and only data-transfer failed
                            */
					pslot->cmd_timeout = 0x0;
					*data_error = ret = -ETIMEDOUT;
	...
	memcpy(&pslot->rw_dma_buf[data_offset], blk_buf, data_blksz);
	...
	unsigned int data_blksz, u32 data_offset, int *data_error)
{
	...
	memcpy(blk_buf, &pslot->rw_dma_buf[data_offset], data_blksz);
	...
	for (i = 0; i < blksz_dwords; i++) {
		memcpy(&x32, &blk_buf[i << 2], sizeof(u32));
				   ...
				   struct mmc_request *mmc_req)
{
	...
	if (req_cmd != NULL) {
		if (req_cmd->data != NULL &&
			...
			pslot->rw_dma_buf =
				kzalloc(req_cmd->data->sg->length * sizeof(u8),
					GFP_DMA);
			if (IS_ERR_OR_NULL(pslot->rw_dma_buf)) {
				dev_err(mmc_dev(ts_sdmmc_host->mmc_host),
			...
			if (pslot->sg_count > 0) {
				dma_unmap_sg(
					mmc_dev(mmc_from_priv(ts_sdmmc_host)),
					req_cmd->data->sg, pslot->sg_count,
					dma_data_dir);
				pslot->sg_count = -1;
			}
...
{
	//struct mmc_host *mmc = ts_sdmmc_host->mmc_host;
	// 	u8 i;
	ts_sdmmc_host->hw_version = ts7800v1_sdmmc_hw_version(ts_sdmmc_host);
	// 	for (i = 0; i < NUM_SD_SLOTS; ++i)
	// 		card_reset(ts_sdmmc_host, i);
...
// {
// 	struct ts7800v1_sdmmc_host *ts_sdmmc_host =
// 		(struct ts7800v1_sdmmc_host *)data;
// 	BUG_ON(irq != ts_sdmmc_host->sdbusy_irq);
// 	trace_printk("%s - irq %d, sdbus %#x\n", __FUNCTION__, irq,
// 		     readl(ts_sdmmc_host->base_iomem + SDCORE2_SDBUS_REG));
// 	return IRQ_HANDLED;
// }
...
static int ts7800v1_sdmmc_probe(struct platform_device *pdev)
{
	...
	ts_sdmmc_host->sdbusy_irq = irq_res->start;
	// 	ret = devm_request_irq(&pdev->dev, ts_sdmmc_host->sdbusy_irq,
	// 			       int_handler, 0x0, "ts_sdmmc", ts_sdmmc_host);
	// 	if (ret) {
	// 		dev_err(&pdev->dev, "Failed to request irq %u (ts_sdmmc)\n",
	// 			ts_sdmmc_host->sdbusy_irq);
	// 		goto devm_request_irq_err;
	// 	}
	...
	/* controller clock frequency capabilities */
	// 	mmc_host->f_init = 400000;
	// 	mmc_host->f_min = 400000;
	// 	mmc_host->f_max = 52000000;

	mmc_host->max_blk_size = MAX_BLK_SIZE;
	// mmc_host->max_blk_count = 0x2000;
	// mmc_host->max_req_size = mmc_host->max_blk_count * mmc_host->max_blk_size;
	// mmc_host->max_req_size = MAX_BLK_SIZE;
	// 	mmc_host->max_seg_size = mmc_host->max_req_size;
...
static int ts7800v1_sdmmc_suspend(struct device *dev)
{
	// 	struct ts7800v1_sdmmc_host *ts_sdmmc_host = dev_get_drvdata(dev);

	// 	if (cancel_delayed_work_sync(&ts_sdmmc_host->timeout_work))
	// 		ts7800v1_sdmmc_request_complete(ts_sdmmc_host, 0);

	// 	ts7800v1_sdmmc_hw_uninit(ts_sdmmc_host);

	dev_info(dev, "TS-7800v1 SD/MMC host controller suspended\n");

	return 0;
}

static int ts7800v1_sdmmc_resume(struct device *dev)
{
	// 	struct ts7800v1_sdmmc_host *ts_sdmmc_host = dev_get_drvdata(dev);

	// 	ts7800v1_sdmmc_host_init(ts_sdmmc_host);

	dev_info(dev, "TS-7800v1 SD/MMC host controller resumed\n");

	return 0;
}
#endif /* CONFIG_PM_SLEEP */

static SIMPLE_DEV_PM_OPS(ts7800v1_sdmmc_pm_ops, ts7800v1_sdmmc_suspend,
			 ts7800v1_sdmmc_resume);
...
MODULE_AUTHOR("Ferar Ashkar <ferar.ashkar@savoirfairelinux.com>");
