...
static int do_ts7800v2_wdt_write(unsigned int val)
{
	...
	ret = i2c_master_send(i2c_client, data, sizeof(data));
...
static int ts7800v2_wdt_write(unsigned int val)
{
	int ret = 0;
	mutex_lock(&wdtlock);
	ret = do_ts7800v2_wdt_write(val);
	mutex_unlock(&wdtlock);
	return ret;
}
